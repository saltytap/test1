...
class FootballProbabilityModel:
    ...
    def over_under_probabilities(self, match: Match, stats: dict[str, TeamStats], line: float) -> dict[str, float]:
        ...
        home = stats[match.home_team]
        away = stats[match.away_team]
        poisson_over = poisson_over_probability(projection.total_goals, line)
        over_history = _mean_optional(home.over25_rate, away.over25_rate)
        profile_over = _clamp(
            (
                _clamp((projection.total_goals - line + 0.85) / 1.70, 0.0, 1.0)
                + _clamp((home.goals_for_avg + away.goals_for_avg) / max((home.league_goals_avg or 2.6) * 1.15, 0.1), 0.0, 1.0)
                + _clamp((home.goals_against_avg + away.goals_against_avg) / max((away.league_goals_avg or 2.6) * 1.15, 0.1), 0.0, 1.0)
            )
            / 3.0,
            0.03,
            0.97,
        )
        if line == 2.5 and over_history is not None:
            over = _clamp((poisson_over * 0.50) + (profile_over * 0.25) + (over_history * 0.25), 0.03, 0.97)
        elif line == 1.5 and over_history is not None:
            adjusted_history = _clamp(over_history + 0.22, 0.03, 0.97)
            over = _clamp((poisson_over * 0.58) + (profile_over * 0.22) + (adjusted_history * 0.20), 0.03, 0.97)
        elif line == 3.5 and over_history is not None:
            adjusted_history = _clamp(over_history - 0.22, 0.03, 0.97)
            over = _clamp((poisson_over * 0.62) + (profile_over * 0.23) + (adjusted_history * 0.15), 0.03, 0.97)
        else:
            over = _clamp((poisson_over * 0.72) + (profile_over * 0.28), 0.03, 0.97)
