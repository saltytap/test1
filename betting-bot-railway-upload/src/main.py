...
def _calibrate_probabilities_to_market(
    ...
    if market.market_type in {"over_under", "btts"}:
    ...
    if preliminary_edges:
        ...
        if average_edge > 0.045:
            model_weight *= 0.78
        if high_edge_share > 0.50:
            model_weight *= 0.70
...
def _calibrate_recommendation_portfolio(
    ...
    for item in recommendations:
        adjusted_probability = item.normalized_implied_probability + (
            (item.model_probability - item.normalized_implied_probability) * shrink_factor
        )
        ...
        tier = _candidate_tier_from_metrics(
            ...
            market_type=_market_type_from_market_name(item.market),
            strong_support=strong_support,
        )
        if tier == "main":
