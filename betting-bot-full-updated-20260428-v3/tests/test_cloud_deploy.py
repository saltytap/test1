...
def test_api_picks_returns_empty_list_when_file_missing(tmp_path) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"last_updated": None, "picks": [], "parlays": []}
...
def test_api_picks_refreshes_when_file_missing_and_key_exists(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("app._generate_latest_picks", fake_generate)
...
def test_api_picks_refreshes_when_existing_file_has_empty_picks(tmp_path, monkeypatch) -> None:
    picks_path = tmp_path / "latest_picks.json"
    picks_path.write_text(json.dumps({"last_updated": "2026-04-27T10:30:06+00:00", "picks": []}), encoding="utf-8")

    def fake_generate(output_path):
        output_path.write_text(
            json.dumps(
                {
                    "last_updated": "2026-04-27T10:40:00+00:00",
                    "picks": [{"match": "Manchester United vs Brentford", "bet_type": "BTTS", "pick": "Yes", "odds": 1.57, "ev": "+0.9%"}],
                }
            ),
            encoding="utf-8",
        )
    ...
    assert response.status_code == 200
    assert response.json()["picks"][0]["match"] == "Manchester United vs Brentford"
